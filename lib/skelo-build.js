...
function buildCategory(item, options) {
    ...
    if (itemType === TOPIC) {
        ...
        return {
            type: 'category',
            label,
            items: buildCategoryItems(items, options)
        };
    }
}
...
/**
 * Constructs category items by filtering and mapping input items based on their type.
 *
 * This function processes an array of items, filtering out invalid items and mapping
 * valid items to their respective structures: category, topic, or link. Invalid items
 * are those that do not match the predefined types: TOPIC, LINK, or CATEGORY.
 *
 * @param {Array} items - The array of items to process.
 * @param {Object} options - Configuration options, potentially used by item constructors.
 * @returns {Array} An array of constructed category items, excluding any invalid items.
 */
