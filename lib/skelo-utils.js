...
/**
 * Analyzes sidebar labels across multiple files to identify duplicates and summarize their usage.
 *
 * This function takes an array of file paths (`validFiles`) and extracts sidebar labels
 * from each file. It then returns an object containing two properties:
 * - `duplicatedSidebarLabels`: An array of sidebar labels that appear more than once across all files.
 * - `sidebarLabelsSummary`: An object summarizing the usage of each sidebar label. The keys are the
 *   sidebar labels, and the values are objects containing the total count of the label across all
 *   files, and a `files` object detailing the files where the label appears and how many times
 *   it appears in each file.
 *
 * @param {string[]} validFiles An array of file paths to analyze.
 * @returns {{duplicatedSidebarLabels: string[], sidebarLabelsSummary: Object.<string, {count: number, files: Object.<string, number>}>}} An object containing the duplicated labels and a summary of label usage.
 * @throws {Error} If `validFiles` is not an array of strings, or if the sidebars from a file are not an array of objects.
 */
...
/**
 * Loads and normalizes sidebar configuration from a YAML file.
 *
 * @param {string} yamlFilename - The path to the YAML file containing sidebar configuration.
 * @returns {Object} An object containing normalized sidebars and any additional properties.
 * @throws {Error} If the yamlFilename is invalid, or if the YAML content is not as expected.
